...
def initial_model_para(
    ...
    with open(os.path.join(savePath_3, "model_para.pkl"), "wb") as f:
        ts_print(f"The parameters setting of model is: {model_para}", symbol = "info")
...
class DenseNetEncoderModel(nn.Module):
    ...
    def __init__(self, n_features, nlayers, n_output, dropout=0.5, growth_rate=0.5):
        ...
        dense_layer_sizes = [
            int(n_features + i * n_features * growth_rate) for i in range(nlayers)
        ]
...
class TiRankModel(nn.Module):
    ...
    def __init__(
        ...
        self.feature_weights = nn.Parameter(
            torch.Tensor(n_features, 1), requires_grad=True
        )
        ...
        if self.encoder_type == "Transformer":
            ...
            self.encoder = DenseNetEncoderModel(n_features, nlayers, n_output, dropout)
        else:
            raise ValueError(f"Unsupported Encoder Type: {self.encoder_type}")

        ## Mode
        if mode == "Cox":
            self.predictor = RiskscorePredictor(n_output, nhid2, n_pred, dropout)

        elif mode == "Regression":
            self.predictor = RegscorePredictor(n_output, nhid2, n_pred, dropout)

        elif mode == "Classification":
            self.predictor = ClassscorePredictor(n_output, nhid2, n_pred, dropout)

        else:
            raise ValueError(f"Unsupported Mode: {mode}")

        self.pathologpredictor = PathologyPredictor(n_output, nhid2, n_patho, dropout)
