...
def PackData(savePath, mode, infer_mode, batch_size=1024):
    ...
    train_dataset_Bulk = BulkDataset(
        train_bulk_gene_pairs_mat, bulkClinical_train, mode=mode
    )
    val_dataset_Bulk = BulkDataset(
        val_bulkExp_gene_pairs_mat, bulkClinical_val, mode=mode
    )
    train_loader_Bulk = DataLoader(
        train_dataset_Bulk, batch_size=batch_size, shuffle=False
    )
    ...
    if infer_mode == "ST":
        ...
        train_loader_SC = DataLoader(
            train_dataset_SC, batch_size=batch_size, shuffle=True
        )
    ...
    for column in train_exp.columns:
        ...
        if geneA in test_exp.index and geneB in test_exp.index:
            ...
            transformed_test_exp[column] = (
                test_exp.loc[geneA] > test_exp.loc[geneB]
            ).astype(int) * 2 - 1

            # Handle cases where geneA == geneB by assigning 0
            transformed_test_exp.loc[:, column][
                test_exp.loc[geneA] == test_exp.loc[geneB]
            ] = 0
        else:
