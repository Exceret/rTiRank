...
def process_embeddings(embeddings, n_components, n_clusters):
    ...
    cluster_labels = KMeans(n_clusters=n_clusters, random_state=0).fit_predict(
        pca_embeddings
    )
...
def plot_patho_class_heatmap(data, save_path):
    ...
    if num_labels > len(colors):
        print(
            f"Warning: Not enough unique colors for {num_labels} clusters. Some colors will be repeated."
        )
